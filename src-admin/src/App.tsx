...
import TabebusdSettings from './Tabs/ebusdSettings';
import TabQueriedDPSettings from './Tabs/QueriedDPSettings';
import TabHistoryDPSettings from './Tabs/HistoryDPSettings';
...
class App extends GenericApp<GenericAppProps, AppState> {
    ...
    async onConnectionReady(): Promise<void> {
        ...
        const aliveState = await this.socket.getState(`system.adapter.ebus.${this.instance}.alive`);
    ...
    onDownloadConfig = (): void => {
        try {
            ...
            const fileName = `ebus-config-instance-${this.instance}.json`;
            a.href = url;
            a.download = fileName;
    ...
    onUploadChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
        ...
        reader.onload = () => {
            try {
                ...
                const parsed = JSON.parse(text) as ebusAdapterConfig;
    ...
    renderMainSettings(): React.JSX.Element {
        ...
        return (
            <TabMainSettings
                ...
                socket={this.socket}
                native={this.state.native as ebusAdapterConfig}
    ...
    renderbusdSettings(): React.JSX.Element {
        if (!this.state.systemConfig) {
            return <div>{I18n.t('Loading...')}</div>;
        }

        console.log("renderbusdSettings called");
    ...
    renderQueriedDPSettings(): React.JSX.Element {
        if (!this.state.systemConfig) {
            return <div>{I18n.t('Loading...')}</div>;
        }

        console.log("renderQueriedDPSettings called");

        return (
            <TabQueriedDPSettings
                alive={this.state.alive}
                common={this.common || ({} as ioBroker.InstanceCommon)}
                socket={this.socket}
                native={this.state.native as ebusAdapterConfig}
    ...
    renderHistoryDPSettings(): React.JSX.Element {
        if (!this.state.systemConfig) {
            return <div>{I18n.t('Loading...')}</div>;
        }

        console.log("renderHistoryDPSettings called");

        return (
            <TabHistoryDPSettings
                alive={this.state.alive}
                common={this.common || ({} as ioBroker.InstanceCommon)}
                socket={this.socket}
                native={this.state.native as ebusAdapterConfig}
                        ...
                        <AppBar position="static">
                            ...
                            <Tabs
                                ...
                                onChange={(_e, value) => {
                                    ...
                                    window.localStorage.setItem(`ebus.${this.instance}.selectedTab`, value);
                                }}
                                variant="scrollable"
                                scrollButtons="auto"
